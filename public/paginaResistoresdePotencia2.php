<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Resistores de Potência</title>
    <link rel="stylesheet" href="../style/styles.css">
    <source src="login.js" type="">
</head>

<body>

    <header>
        <div id="barraescura">
            <a href="paginaResistoresdePotencia.html"><img class="topo1" src="../asets/imagens/barraAcima/flecha.png" alt=""></a>
            <img class="topo2" src="../asets/imagens/barraAcima/tradutor.png" alt="">
        </div>
    </header>
    <br>
    <br>
    <main>
        <div id="azul">
            <h2 id="hs">Resistores de Potência</h2>
        </div>
        <br>
        <br>

        <div class="redonda3">
            <p>Falhas térmicas</p><p class="red"><strong>°1</strong></p>▼</p>
        </div>

        <div class="informacao3">
            <p>Risco de sobreaquicimento da linha 9, que pode ocasionar em deformação física. </p>
        </div>

        <div class="redonda3">
            <p>Problemas estruturais</p><p class="red"><strong>°2</strong></p>▼
        </div>

        <div class="informacao3">
            <p>Desgaste dos resitores de carbono na linha 10</p>
            <p>Falha na dissipação de calor da linha 5, está causando o acúmulo de calor.</p>
        </div>

        <div class="redonda3">
            <p>Erros de Projeto</p><p class="red"><strong>°1</strong></p>▼
        </div>

        <div class="informacao3">
            <p>Na linha 6 foi identificado um uso de resistor com potência inferior ás necessidades do circuito.</p>
        </div>

        <br>
        <br>
    </main>

    <footer>
        <div id="barra">
            <a href="paginainformacoes.html"><img class="topo1" src="../asets/imagens/barraAbaixo/barras.png" alt=""></a>
            <img class="logo" src="../asets/imagens/barraAbaixo/logo.png" alt="">
            <h3>Fast.sesi</h3>
        </div>
    </footer>

</body>

</html>


